namespace Adom.Analyzers.Test
{
    ...
    {
        private readonly T _diagnosticAnalyzer;

        protected CSharpDiagnosticVerifier()
        {
            this._diagnosticAnalyzer = new T();
        }

        public T DiagnosticAnalyzer { get => this._diagnosticAnalyzer; }
        ...
        protected void VerifyCSharpDiagnostic(TestSourceFile source, params DiagnosticResult[] expected)
        {
            VerifyDiagnostics(new[] { source }, LanguageNames.CSharp, this._diagnosticAnalyzer, expected);
        }
        ...
        protected void VerifyCSharpDiagnostic(TestSourceFile[] sources, params DiagnosticResult[] expected)
        {
            VerifyDiagnostics(sources, LanguageNames.CSharp, this._diagnosticAnalyzer, expected);
        }
        ...
        /// <param name="expected">DiagnosticResults that should appear after the analyzer is run on the sources</param>
        private static void VerifyDiagnostics(TestSourceFile[] sources, string language, DiagnosticAnalyzer analyzer, params DiagnosticResult[] expected)
        {
        ...
        #region Comparison and verification 
        ...
        private static void VerifyDiagnosticLocation(DiagnosticAnalyzer analyzer, Diagnostic diagnostic, Location actual, DiagnosticResultLocation expected)
        {
            ...
            Assert.True(
                actualSpan.Path == expected.Path || (actualSpan.Path != null && actualSpan.Path.Contains("Test0.", StringComparison.InvariantCulture) && expected.Path.Contains("Test.", StringComparison.InvariantCulture)),
